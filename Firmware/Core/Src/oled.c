...
static void OLED_WriteData(uint8_t dat)//只能告诉亮还是不亮
{
    uint8_t data[2];
...
void OLED_SetCursor(uint8_t page, uint8_t x)//这里是设置在哪里写字的，高度和长度
{
...
void OLED_ShowChinese(uint8_t page, uint8_t x, uint8_t index)
{
    uint8_t i;
    ...
    for (i = 0; i < 16; i++)
    {
        OLED_WriteData(OLED_Hz16[index][i]);
    }
    ...
    for (i = 0; i < 16; i++)
    {
        OLED_WriteData(OLED_Hz16[index][i + 16]);
    }
}
void OLED_Init(void)
{
